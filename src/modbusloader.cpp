...
/*
void ModbusLoader::Crc16(char* pucFrame, int usLen) {
    int  iIndex;
    while( usLen-- )
    {
...
void ModbusLoader::executeStateMachine()
{
    ...
    case State::VerifyFlash:
        ...
        break;
/*  
   case State::VerifyFlash16:
        if (checksumFirmware16 == modbusClient->getResult().at(0)) {
            emit newMessageAvailable(
                    u"Flash verified (checksum16 = 0x%1)"_s.arg(checksumFirmware16, 0, 16));
            state = State::WriteCRC16;
        } else {
            errorStr = u"Verifying flash failed"_s;
            emit finished(false);
        }
...
*/
/*
    case State::WriteCRC16:
        if (modbusClient->getResult().front() == 0) {
            state = State::ResetDevice;
            emit newMessageAvailable(u"CRC16 writed"_s);
        } else {
            errorStr = u"Writing CRC16 failed"_s;
            emit finished(false);
        }
        break;
*/
    ...
    case State::ProgramFlash: {
        ...
        //Crc16(firmwareTemp, readCount);
        //checksumFirmware16=( ucCRCHi << 8 | ucCRCLo );
...
/*
    case State::VerifyFlash16:
        emit newMessageAvailable(u"Verifying flash crc16..."_s);
        modbusClient->modbusCustomFunction(ModbusCustomClient::Subfunctions::GetChecksum16Flash);
        break;

    case State::WriteCRC16:
        emit newMessageAvailable(u"Write CRC..."_s);
        modbusClient->modbusCustomFunction(ModbusCustomClient::Subfunctions::WriteCRC16);
        break;
*/
